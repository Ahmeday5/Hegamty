...
export interface Toast {
  ...
  closable: boolean;
  /** Optional inline action (e.g. "Undo", "View"). */
  action?: ToastAction;
 ...
 *
 *   toast.success('Saved');
 *   toast.error('Connection failed', { title: 'Error', action: { label: 'Retry', handler: retry } });
 *   toast.info('Sending…', { duration: 0 }); // sticky
 *
...
export class ToastService {
  ...
  /** RTL app — toasts sit at the top-left and slide in from that edge. */
